 ...
 *
 * The reliable auto-expand mechanism is to hook onLayout and performClick() the real
 * system expand arrow (android:expand_button / NotificationExpandButton) — setUserExpanded
 * alone does not stick. We NEVER force size/height/visible-type for shade/LS; only heads-up
 * has dedicated height/visible-type hooks. On keyguard the expand click is routed through
 * clickExpandSilentlyOnKeyguard so the onExpandClicked -> locked-shade scrim/dim/unlock side
 * effect is suppressed. aeAutoExpanded is a per-row one-shot with a 300ms quiet-gap reset =
 * exactly one expand per open and no relayout feedback loop.
 *
 * This is a behavior-preserving extraction from MainHook — the hook bodies are verbatim.
 */
class NotificationExpander(private val prefs: PrefsBridge) {
